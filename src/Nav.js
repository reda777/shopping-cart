...
function Nav({cart,setCart}){
    const cartSVG=(<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    return (
        <nav>
            ...
            <div data-testid="toggleCart" className="toggleCart" title="Cart" onClick={toggleShowCart}>
                <div className="cartSVG">
                    {cartSVG}
                </div>
            </div>
            {(showCart)?<Cart cart={cart} setCart={setCart} />:null}
        </nav>
    );
}
export default Nav;