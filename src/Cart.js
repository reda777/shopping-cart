...
function Cart(props) {
  ...
  function handlePlus(e) {
    const newCart = props.cart.map((prod) => {
      if (
        prod.id === parseInt(e.currentTarget.dataset.id) &&
  ...
  function handleRemove(e) {
    let newCart = props.cart.filter(
      (prod) => prod.id !== parseInt(e.currentTarget.dataset.id)
    );
    props.setCart(newCart);
  }
  return (
    <div className="cart" data-testid="cart">
        ...
        {props.cart.map((prod) => {
          return (
            <div data-testid="prod" className="prod" key={prod.id}>
              ...
              <div className="price">
                Price: <p> &nbsp;{prod.price * prod.quantity}</p>
              </div>
              <div
      ...
      {props.cart.length === 0 ? (
        <div data-testid="noItems" className="checkoutBtn">
          <div className="noItems">Checkout</div>
        </div>
      ) : (
        <div data-testid="checkoutBtn" className="checkoutBtn">
